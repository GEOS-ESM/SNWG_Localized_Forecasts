#!/usr/bin/env python3
# ---------------------------------------------------------------------------
# PROJ database fix: must happen before any import of rasterio/pyproj so that
# those libraries find the compatible proj.db bundled inside the rasterio wheel
# rather than the stale one that conda may have placed on PROJ_LIB / PROJ_DATA.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
"""
GEOS-CF NetCDF4 → Daily-Average GeoTIFF converter
====================================================
Downloads hourly aqc_tavg_1hr_glo_L1440x721_slv forecast files from the
NASA GMAO GEOS-CF portal, computes 24-hour (UTC) means for PM2.5 and NO2,
and writes one GeoTIFF per variable per forecast day (5 days by default).

Variables exported by default:
    PM25_RH35  – PM2.5 (μg m⁻³, at RH 35%)
    NO2        – NO2   (mol mol⁻¹)

Usage examples
--------------
# Full 5-day run (auto-detected init date):
    python3 geos_cf_to_tiff.py

# Quick local test – downloads ONE hour, writes two single-hour TIFFs:
    python3 geos_cf_to_tiff.py --test

# Specify an explicit forecast-init date (YYYYMMDD) and init hour:
    python3 geos_cf_to_tiff.py --init-date 20260312 --init-hour 09

# Keep downloaded .nc4 files after conversion:
    python3 geos_cf_to_tiff.py --keep-nc4

# Write TIFFs to a custom directory:
    python3 geos_cf_to_tiff.py --output-dir precomputed/pmtiles_output

Output file naming
------------------
    geos_cf_<VAR>_<YYYYMMDD>_<init_hour>z.tif          (daily average)
    geos_cf_<VAR>_test_<YYYYMMDD>_<HH>z.tif            (--test single hour)
    e.g.  geos_cf_PM25_RH35_GCC_20260312_09z.tif
...
# aqc_tavg_1hr collection: small (~6 MB), surface-level, hourly averages
COLLECTION = "aqc_tavg_1hr_glo_L1440x721_slv"

# Default variables to extract from the NetCDF files (PM2.5 and NO2 only)
DEFAULT_VARIABLES = ["PM25_RH35", "NO2"]

# Forecast length in hours (GEOS-CF runs ~5 days, 120 hours, from 09z)
FORECAST_LENGTH_HOURS = 120

# The GEOS-CF grid is global 1440×721 (0.25° resolution)
#   lon: -180 → 180  (1440 points, centre of first cell = -179.875)
#   lat:  -90 → +90  (721 points, centre of first cell = -90.0)
GRID_NX = 1440
GRID_NY = 721
GRID_WEST = -180.0
GRID_EAST =  180.0
...
def build_file_url(init_date: str, init_hour: str, valid_dt: datetime) -> str:
    """
    Build the full HTTPS URL for one hourly aqc_tavg file.

    Parameters
    ----------
    init_date : str   YYYYMMDD of the forecast-init date
    init_hour : str   two-digit hour string, e.g. "09"
    valid_dt  : datetime  valid time (UTC, timezone-aware or naive)

    Returns
    -------
    str  full URL
    """
    # valid time in filename uses HHMMz notation (half-hour centre of window)
...
def latest_init_date_and_hour() -> Tuple[str, str]:
    """
    Return the most-recently-available forecast init date (YYYYMMDD) and hour.
    GEOS-CF runs once daily at 09z; we back off up to 3 days to find a run
    that actually has files on the server.
    """
    now_utc = datetime.now(timezone.utc)
    for days_back in range(4):
        candidate = now_utc - timedelta(days=days_back)
        date_str  = candidate.strftime("%Y%m%d")
        # probe the first file of the run
...
def extract_variables(nc_path: Path, variables: list[str]) -> dict[str, np.ndarray]:
    """
    Open a GEOS-CF .nc4 file and return a dict of {var_name: 2-D array}.
    The returned arrays have shape (NY, NX) = (721, 1440) in float32.
    ...
    with nc4.Dataset(nc_path, "r") as ds:
        for var in variables:
            if var not in ds.variables:
                ...
                continue
            raw = ds.variables[var][:]          # masked array, shape (1, NY, NX) or (1, lev, NY, NX)
            arr = np.ma.filled(raw, fill_value=np.nan).squeeze()
            # If a lev dimension remains after squeeze (3-D), take surface level (index 0)
            if arr.ndim == 3:
                arr = arr[0]
            ...
            if arr.ndim != 2 or arr.shape != (GRID_NY, GRID_NX):
                ...
                continue
            # Flip latitude axis if needed: rasterio expects top→bottom (N→S)
            # GEOS-CF stores data bottom→top (S→N) based on lat coordinate
            # Check the lat dimension direction and flip if ascending (S→N)
            if "lat" in ds.variables:
                lats = ds.variables["lat"][:]
                if lats[0] < lats[-1]:      # ascending = S→N → flip to N→S
                    arr = np.flipud(arr)
            out[var] = arr
    return out
...
def compute_daily_averages(
    ...
    """
    Download all hourly files that fall within *forecast_day* (UTC midnight→midnight),
    stack their arrays, and return the 24-hour mean for each variable.

    Returns dict {var: mean_array(721, 1440)}  or {} if no files were found.
    """
    # Determine valid times for this day (00z–23z, i.e. hours 0..23)
    day_start = datetime(
    ...
    for var, arrays in stacks.items():
        if arrays:
            stk = np.stack(arrays, axis=0)          # (N, 721, 1440)
            means[var] = np.nanmean(stk, axis=0)    # (721, 1440)
    return means
...
def write_geotiff(
    ...
    """
    Write a multi-band Cloud-Optimised GeoTIFF.

    Band order follows *variables* (only bands present in *data* are written).
    Metadata includes band descriptions and variable names.
    Optional *extra_tags* are merged into the TIFF metadata.
    """
    ...
    crs = CRS.from_wkt(
        ...
        'AUTHORITY["EPSG","9122"]],'
        'AUTHORITY["EPSG","4326"]]'
    )  # WGS 84 – inline WKT avoids any PROJ database lookup
...
def write_manifest(output_dir: Path) -> None:
    """
    Scan *output_dir* for all .tif / .tiff files and write a
    ``layers_manifest.json`` that the JS front-end reads to populate
    the layer dropdown automatically.

    Each entry has:
        file      – filename (relative, no path)
        path      – relative path served by the web server
        name      – human-readable label
        pollutant – 'pm25', 'no2', or 'default'
        type      – 'geotiff'
        unit      – display unit string
        date      – ISO date string or null
        test      – true if this is a single-hour test TIFF
    """
    # Variable → (pollutant key, unit label)
    ...
    # Regex for generated filenames:
    # geos_cf_<VAR>_<YYYYMMDD>_<HH>z.tif          (daily average)
    # geos_cf_<VAR>_test_<YYYYMMDD>_<HH>z.tif     (test single-hour)
    ...
    for f in sorted(output_dir.iterdir()):
        ...
        if var_raw:
            # Match var_raw back to a known variable (handle safe_var substitution)
            ...
            # Unknown naming convention — still include the file
...
def run(
    ...
    """
    Full pipeline: download → average → write TIFFs for *days* forecast days.
    """
    ...
    # init datetime in UTC
    init_dt = datetime(
        int(init_date[:4]), int(init_date[4:6]), int(init_date[6:8]),
        int(init_hour), 0, tzinfo=timezone.utc
    )

    # Compute which forecast days are fully covered by the run
    # The run starts at init_hour (09z) on init_date and covers ~5 days.
    # Day 0 of forecast = the day of init; first *complete* 24-h day is Day 1.
    # We include Day 0 (partial) through Day N.
    forecast_days = [
        (init_dt + timedelta(days=d)).replace(hour=0, minute=0, second=0)
        for d in range(days)
    ]

    # Use a temp directory for downloaded .nc4 files (unless keep_nc4)
    tmp_ctx = (
...
def run_test(
    ...
    """
    Test mode: download a single hourly file (init + 1 h) and write one
    single-hour TIFF per variable.

    Output filename pattern:
        geos_cf_<VAR>_test_<YYYYMMDD>_<HH>z.tif
    """
...
def list_variables(init_date: str, init_hour: str) -> None:
    """Download a single file and print all NetCDF variables."""
    init_dt = datetime(
        int(init_date[:4]), int(init_date[4:6]), int(init_date[6:8]),
        int(init_hour), 0, tzinfo=timezone.utc
    )
